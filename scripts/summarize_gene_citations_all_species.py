...
for taxonomy_pair in taxonomy_list:
    ...
    elif "genbank common name" in taxonomy_pair[2]:
        taxonomy_dict[taxonomy_pair[0]] = taxonomy_pair[1]
...
for values in string_values:
    gene_id = values.split("\t")[0]
    if gene_id not in gene_id_dict.keys():
        gene_id_dict[gene_id] = [values]
    else: 
        gene_id_dict[gene_id].append(values)
...
for gene_id_tax_id in gene_id_dict.keys():
    ...
    if gene_id_tax_id in taxonomy_dict.keys():
        gene_species_name = taxonomy_dict[gene_id_tax_id].replace(" ", "_")
...
with open('top_ten_citation_per_species.html', 'w') as f:
    ...
    for tile_name in top_ten_citation.keys():
        ...
        html_text = html_text + "<tr> <th>Tax_ID</th> <th>Gene_ID</th> <th>YEAR_Searched</th> <th>Gene_name</th> <th>Gene_info</th> <th>Gene_Type</th> <th>Citation</th></tr>"
